...
var EVMPKR = class {
  ...
  async getSCAddress(stub) {
    ...
    const scAsBytes = await stub.getState('SCADDRESS'); // get the car from chaincode state
  ...
  async getEVMAddress(stub) {
    ...
    const evmAsBytes = await stub.getState('EVMADDRESS'); // get the car from chaincode state
